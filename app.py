...
# Bloqueia extensões de tradução para evitar conflito de renderização
...
def grafico_com_variacao(dados_linha_df, tickers_nomes: dict, cores, var_dict: dict, mostrar_legenda: bool = False):
    fig = go.Figure()
    cols_existentes = [t for t in tickers_nomes.keys() if t in dados_linha_df.columns]
    if not cols_existentes:
        return fig
        
    df_filtrado = dados_linha_df[cols_existentes].dropna(how='all')
...
# ------------------------------------------------------------------
# FRAGMENTO DE AUTO-REFRESH (Substitui o st_autorefresh com estabilidade)
# ------------------------------------------------------------------
...
def renderizar_painel():
    ...
    with col_esquerda:
        ...
        with c_g1:
            st.plotly_chart(
                grafico_com_variacao(dados_linha, MOEDAS, CORES_MOEDAS_EXATAS, dados_var, mostrar_legenda=True), 
    ...
    with col_direita:
        ...
        with tab_us:
            ...
            with c_g2:
                st.plotly_chart(
                    grafico_com_variacao(dados_linha, YIELDS, CORES_YIELDS, dados_var, mostrar_legenda=False), 
                    use_container_width=True,
                    key="graf_yields"
                )
            with c_t2:
        ...
        with tab_di:
            ...
            with c_g3:
                st.plotly_chart(
                    grafico_com_variacao(dados_linha, DI_B3, CORES_DI, dados_var, mostrar_legenda=False), 
                    use_container_width=True,
                    key="graf_di"
                )
            with c_t3:
    ...
    with col_adr:
        ...
        st.plotly_chart(fig_adrs_bar, use_container_width=True, key="graf_adrs")
    ...
    with col_macro:
        ...
        st.plotly_chart(fig_comm_bar, use_container_width=True, key="graf_macro")
